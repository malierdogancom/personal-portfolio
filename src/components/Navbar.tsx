...
export default function Navbar() {
    ...
    const LangToggle = () => (
        <div className="flex items-center gap-2 border border-line rounded-full px-3 py-1 bg-secondary/40">
            <button
        ...
        <nav className="border-b border-secondary bg-background/80 backdrop-blur-md sticky top-0 z-50">
            <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
                ...
                <div className="md:hidden flex items-center gap-3">
                    <LangToggle />
                    <button onClick={toggleMenu} className="p-2 text-muted-foreground hover:text-primary transition-colors" aria-label="Toggle menu" aria-expanded={isMenuOpen}>
                        {isMenuOpen ? (
            ...
            {isMenuOpen && (
                <div className="md:hidden border-t border-secondary bg-background/95 backdrop-blur-md absolute w-full left-0 top-16 shadow-lg">
                    <div className="flex flex-col p-6 gap-4 text-center font-medium" onClick={closeMenu}>
                        {links}
                    </div>
                </div>
            )}
